...
public class KeyAgreementSpi extends BaseAgreementSpi {
    ...
    protected byte[] e(BigInteger bigInteger) {
        int bitLength = (this.f13626p.bitLength() + 7) / 8;
    ...
    protected Key engineDoPhase(Key key, boolean z) {
        if (this.x != null) {
            if (key instanceof DHPublicKey) {
                ...
                if (dHPublicKey.getParams().getG().equals(this.f13625g) && dHPublicKey.getParams().getP().equals(this.f13626p)) {
                    BigInteger y = dHPublicKey.getY();
                    if (y != null && y.compareTo(TWO) >= 0) {
                        BigInteger bigInteger = this.f13626p;
                        ...
                        if (y.compareTo(bigInteger.subtract(bigInteger2)) < 0) {
                            if (this.unifiedAgreement != null) {
                                ...
                                if (z) {
                                    this.result = e(this.mqvAgreement.calculateAgreement(new DHMQVPublicParameters(generatePublicKeyParameter((PublicKey) key), generatePublicKeyParameter(this.mqvParameters.getOtherPartyEphemeralKey()))));
                                    return null;
                                }
                                throw new IllegalStateException("MQV Diffie-Hellman can use only two key pairs");
                            } else {
                                BigInteger modPow = y.modPow(this.x, this.f13626p);
                                if (modPow.compareTo(bigInteger2) != 0) {
                                    this.result = e(modPow);
                                    if (z) {
                                        return null;
                                    }
    ...
    public SecretKey engineGenerateSecret(String str) {
        if (this.x != null) {
            return str.equals("TlsPremasterSecret") ? new SecretKeySpec(BaseAgreementSpi.d(this.result), str) : super.engineGenerateSecret(str);
        }
        throw new IllegalStateException("Diffie-Hellman not initialised.");
    }
    ...
    protected void engineInit(Key key, SecureRandom secureRandom) {
        ...
        this.f13626p = dHPrivateKey.getParams().getP();
        this.f13625g = dHPrivateKey.getParams().getG();
    ...
    protected void engineInit(Key key, AlgorithmParameterSpec algorithmParameterSpec, SecureRandom secureRandom) {
        ...
        if (algorithmParameterSpec == null) {
            ...
            this.f13626p = dHParameterSpec.getP();
            this.f13625g = dHParameterSpec.getG();
