...
public class DSTU4145Signer implements DSAExt {
    ...
    public BigInteger[] generateSignature(byte[] bArr) {
        ...
        ECMultiplier a2 = a();
        while (true) {
            BigInteger generateRandomInteger = generateRandomInteger(n2, this.random);
            ECFieldElement affineXCoord = a2.multiply(parameters.getG(), generateRandomInteger).normalize().getAffineXCoord();
            if (!affineXCoord.isZero()) {
                BigInteger fieldElement2Integer = fieldElement2Integer(n2, hash2FieldElement.multiply(affineXCoord));
                if (fieldElement2Integer.signum() != 0) {
                    BigInteger mod = fieldElement2Integer.multiply(d2).add(generateRandomInteger).mod(n2);
                    if (mod.signum() != 0) {
    ...
    public boolean verifySignature(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        return !normalize.isInfinity() && fieldElement2Integer(n2, hash2FieldElement.multiply(normalize.getAffineXCoord())).compareTo(bigInteger) == 0;
    }
}
