...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    private BigInteger bitsToInt(byte[] bArr) {
        ...
        return bArr.length * 8 > this.f13518n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f13518n.bitLength()) : bigInteger;
    }

    @Override // org.bouncycastle.crypto.signers.DSAKCalculator
    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f13518n = bigInteger;
    ...
    public BigInteger nextK() {
        int unsignedByteLength = BigIntegers.getUnsignedByteLength(this.f13518n);
        ...
        while (true) {
            ...
            while (i2 < unsignedByteLength) {
                ...
                this.hMac.doFinal(this.V, 0);
                int min = Math.min(unsignedByteLength - i2, this.V.length);
                System.arraycopy(this.V, 0, bArr, i2, min);
                i2 += min;
            }
            BigInteger bitsToInt = bitsToInt(bArr);
            if (bitsToInt.compareTo(ZERO) > 0 && bitsToInt.compareTo(this.f13518n) < 0) {
                return bitsToInt;
            }
            HMac hMac2 = this.hMac;
            byte[] bArr3 = this.V;
