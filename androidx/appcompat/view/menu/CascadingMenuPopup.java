...
final class CascadingMenuPopup extends MenuPopup implements View.OnKeyListener, PopupWindow.OnDismissListener {
    ...
    final List f392b = new ArrayList();

    /* renamed from: c  reason: collision with root package name */
    final ViewTreeObserver.OnGlobalLayoutListener f393c = new ViewTreeObserver.OnGlobalLayoutListener() { // from class: androidx.appcompat.view.menu.CascadingMenuPopup.1
        @Override // android.view.ViewTreeObserver.OnGlobalLayoutListener
        public void onGlobalLayout() {
            if (!CascadingMenuPopup.this.isShowing() || CascadingMenuPopup.this.f392b.size() <= 0 || ((CascadingMenuInfo) CascadingMenuPopup.this.f392b.get(0)).window.isModal()) {
                return;
            }
            View view = CascadingMenuPopup.this.f394d;
            if (view == null || !view.isShown()) {
                CascadingMenuPopup.this.dismiss();
                return;
            }
            for (CascadingMenuInfo cascadingMenuInfo : CascadingMenuPopup.this.f392b) {
    ...
    private final View.OnAttachStateChangeListener mAttachStateChangeListener = new View.OnAttachStateChangeListener() { // from class: androidx.appcompat.view.menu.CascadingMenuPopup.2
        ...
        public void onViewDetachedFromWindow(View view) {
            ViewTreeObserver viewTreeObserver = CascadingMenuPopup.this.f395e;
            if (viewTreeObserver != null) {
                if (!viewTreeObserver.isAlive()) {
                    CascadingMenuPopup.this.f395e = view.getViewTreeObserver();
                }
                CascadingMenuPopup cascadingMenuPopup = CascadingMenuPopup.this;
                cascadingMenuPopup.f395e.removeGlobalOnLayoutListener(cascadingMenuPopup.f393c);
            }
    ...
    private final MenuItemHoverListener mMenuItemHoverListener = new MenuItemHoverListener() { // from class: androidx.appcompat.view.menu.CascadingMenuPopup.3
        ...
        public void onItemHoverEnter(@NonNull final MenuBuilder menuBuilder, @NonNull final MenuItem menuItem) {
            CascadingMenuPopup.this.f391a.removeCallbacksAndMessages(null);
            int size = CascadingMenuPopup.this.f392b.size();
            int i2 = 0;
            while (true) {
                if (i2 >= size) {
                    i2 = -1;
                    break;
                } else if (menuBuilder == ((CascadingMenuInfo) CascadingMenuPopup.this.f392b.get(i2)).menu) {
                    break;
                } else {
                    i2++;
                }
            ...
            int i3 = i2 + 1;
            final CascadingMenuInfo cascadingMenuInfo = i3 < CascadingMenuPopup.this.f392b.size() ? (CascadingMenuInfo) CascadingMenuPopup.this.f392b.get(i3) : null;
            CascadingMenuPopup.this.f391a.postAtTime(new Runnable() { // from class: androidx.appcompat.view.menu.CascadingMenuPopup.3.1
                @Override // java.lang.Runnable
                public void run() {
                    CascadingMenuInfo cascadingMenuInfo2 = cascadingMenuInfo;
                    if (cascadingMenuInfo2 != null) {
                        CascadingMenuPopup.this.f396f = true;
                        cascadingMenuInfo2.menu.close(false);
                        CascadingMenuPopup.this.f396f = false;
                    }
        ...
        public void onItemHoverExit(@NonNull MenuBuilder menuBuilder, @NonNull MenuItem menuItem) {
            CascadingMenuPopup.this.f391a.removeCallbacksAndMessages(menuBuilder);
        }
    };
    ...
    private int findIndexOfAddedMenu(@NonNull MenuBuilder menuBuilder) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            if (menuBuilder == ((CascadingMenuInfo) this.f392b.get(i2)).menu) {
                return i2;
            }
        }
        return -1;
    ...
    private int getNextMenuPosition(int i2) {
        ...
        Rect rect = new Rect();
        this.f394d.getWindowVisibleDisplayFrame(rect);
    ...
    private void showMenu(@NonNull MenuBuilder menuBuilder) {
        ...
        int b2 = MenuPopup.b(menuAdapter, null, this.mContext, this.mMenuMaxWidth);
        MenuPopupWindow createPopupWindow = createPopupWindow();
        createPopupWindow.setAdapter(menuAdapter);
        createPopupWindow.setContentWidth(b2);
        ...
        if (view != null) {
            ...
            int nextMenuPosition = getNextMenuPosition(b2);
        ...
        this.f392b.add(new CascadingMenuInfo(createPopupWindow, menuBuilder, this.mLastPosition));
    ...
    public void dismiss() {
        int size = this.f392b.size();
        if (size > 0) {
            CascadingMenuInfo[] cascadingMenuInfoArr = (CascadingMenuInfo[]) this.f392b.toArray(new CascadingMenuInfo[size]);
    ...
    public boolean isShowing() {
        return this.f392b.size() > 0 && ((CascadingMenuInfo) this.f392b.get(0)).window.isShowing();
    }
    ...
    public void onCloseMenu(MenuBuilder menuBuilder, boolean z) {
        ...
        if (i2 < this.f392b.size()) {
            ((CascadingMenuInfo) this.f392b.get(i2)).menu.close(false);
        }
        CascadingMenuInfo cascadingMenuInfo = (CascadingMenuInfo) this.f392b.remove(findIndexOfAddedMenu);
        ...
        cascadingMenuInfo.window.dismiss();
        int size = this.f392b.size();
        this.mLastPosition = size > 0 ? ((CascadingMenuInfo) this.f392b.get(size - 1)).position : getInitialMenuPosition();
        if (size != 0) {
            if (z) {
                ((CascadingMenuInfo) this.f392b.get(0)).menu.close(false);
                return;
            }
            return;
        }
        ...
        ViewTreeObserver viewTreeObserver = this.f395e;
        if (viewTreeObserver != null) {
            if (viewTreeObserver.isAlive()) {
                this.f395e.removeGlobalOnLayoutListener(this.f393c);
            }
            this.f395e = null;
        }
        this.f394d.removeOnAttachStateChangeListener(this.mAttachStateChangeListener);
    ...
    public void onDismiss() {
        ...
        while (true) {
            ...
            cascadingMenuInfo = (CascadingMenuInfo) this.f392b.get(i2);
            if (!cascadingMenuInfo.window.isShowing()) {
                break;
            }
            i2++;
    ...
    public boolean onSubMenuSelected(SubMenuBuilder subMenuBuilder) {
        for (CascadingMenuInfo cascadingMenuInfo : this.f392b) {
    ...
    public void show() {
        ...
        this.f394d = view;
        if (view != null) {
            boolean z = this.f395e == null;
            ViewTreeObserver viewTreeObserver = view.getViewTreeObserver();
            this.f395e = viewTreeObserver;
            if (z) {
                viewTreeObserver.addOnGlobalLayoutListener(this.f393c);
            }
            this.f394d.addOnAttachStateChangeListener(this.mAttachStateChangeListener);
        }
    }
