...
public class LinearCurveFit extends CurveFit {
    ...
    public LinearCurveFit(double[] dArr, double[][] dArr2) {
        ...
        this.f1796a = new double[length2];
        this.mT = dArr;
        this.mY = dArr2;
        if (length2 > 2) {
            int i2 = 0;
    ...
    public void getPos(double d2, double[] dArr) {
        ...
        if (this.mExtrapolate) {
            if (d2 <= dArr2[0]) {
                getSlope(dArr2[0], this.f1796a);
                for (int i3 = 0; i3 < length2; i3++) {
                    dArr[i3] = this.mY[0][i3] + ((d2 - this.mT[0]) * this.f1796a[i3]);
                }
                return;
            }
            int i4 = length - 1;
            if (d2 >= dArr2[i4]) {
                getSlope(dArr2[i4], this.f1796a);
                while (i2 < length2) {
                    dArr[i2] = this.mY[i4][i2] + ((d2 - this.mT[i4]) * this.f1796a[i2]);
                    i2++;
                }
                return;
            }
    ...
    public void getPos(double d2, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            if (d2 <= dArr[0]) {
                getSlope(dArr[0], this.f1796a);
                for (int i3 = 0; i3 < length2; i3++) {
                    fArr[i3] = (float) (this.mY[0][i3] + ((d2 - this.mT[0]) * this.f1796a[i3]));
                }
                return;
            }
            int i4 = length - 1;
            if (d2 >= dArr[i4]) {
                getSlope(dArr[i4], this.f1796a);
                while (i2 < length2) {
                    fArr[i2] = (float) (this.mY[i4][i2] + ((d2 - this.mT[i4]) * this.f1796a[i2]));
                    i2++;
                }
                return;
            }
