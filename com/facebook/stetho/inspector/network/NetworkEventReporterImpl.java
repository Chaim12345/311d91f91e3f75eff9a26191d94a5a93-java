...
public class NetworkEventReporterImpl implements NetworkEventReporter {
    ...
    static AsyncPrettyPrinter createPrettyPrinterForResponse(NetworkEventReporter.InspectorResponse inspectorResponse, @Nullable AsyncPrettyPrinterRegistry asyncPrettyPrinterRegistry) {
        if (asyncPrettyPrinterRegistry != null) {
            ...
            for (int i2 = 0; i2 < headerCount; i2++) {
                AsyncPrettyPrinterFactory lookup = asyncPrettyPrinterRegistry.lookup(inspectorResponse.headerName(i2));
                if (lookup != null) {
                    return lookup.getInstance(inspectorResponse.headerName(i2), inspectorResponse.headerValue(i2));
                }
            }
            return null;
        }
        ...
        for (int i2 = 0; i2 < inspectorHeaders.headerCount(); i2++) {
            ...
            try {
                if (jSONObject.has(headerName)) {
                    headerValue = jSONObject.getString(headerName) + "\n" + headerValue;
                }
    ...
    private static String readBodyAsString(NetworkPeerManager networkPeerManager, NetworkEventReporter.InspectorRequest inspectorRequest) {
        try {
            ...
            CLog.writeToConsole(networkPeerManager, messageLevel, messageSource, "Could not reproduce POST body: " + e2);
            return null;
        }
    }
