...
public class ObjectMapper {
    ...
    private List<Object> convertArrayToList(Field field, JSONArray jSONArray) {
        ...
        for (int i2 = 0; i2 < jSONArray.length(); i2++) {
            if (cls.isEnum()) {
                convertValue = getEnumValue(jSONArray.getString(i2), cls);
            } else if (canDirectlySerializeClass(cls)) {
                convertValue = jSONArray.get(i2);
            } else {
                JSONObject jSONObject = jSONArray.getJSONObject(i2);
                convertValue = jSONObject == null ? null : convertValue(jSONObject, cls);
            }
    ...
    private static Method getJsonValueMethodImpl(Class<?> cls) {
        ...
        for (int i2 = 0; i2 < methods.length; i2++) {
            if (methods[i2].getAnnotation(JsonValue.class) != null) {
                return methods[i2];
            }
        }
        return null;
    ...
    private Object getValueForField(Field field, Object obj) {
        if (obj != null) {
            try {
                ...
                throw new IllegalArgumentException("Unable to set value for field " + field.getName(), e2);
            }
        }
        return obj;
    }
