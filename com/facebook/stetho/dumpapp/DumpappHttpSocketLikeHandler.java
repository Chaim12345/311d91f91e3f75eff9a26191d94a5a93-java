    ...
    private static class DumpappLegacyHttpHandler implements HttpHandler {
        ...
        public boolean handleRequest(SocketLike socketLike, LightHttpRequest lightHttpRequest, LightHttpResponse lightHttpResponse) {
            ...
            if (z || equals) {
                ...
                create = LightHttpBody.create(byteArrayOutputStream.toByteArray(), "application/octet-stream");
            } else {
                lightHttpResponse.code = 501;
                lightHttpResponse.reasonPhrase = "Not implemented";
                create = LightHttpBody.create(lightHttpRequest.method + " not implemented", HTTP.PLAIN_TEXT_TYPE);
            }
            lightHttpResponse.body = create;
            return true;
        }
    }
