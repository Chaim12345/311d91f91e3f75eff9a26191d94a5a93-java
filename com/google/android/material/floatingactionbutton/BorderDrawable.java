...
public class BorderDrawable extends Drawable {
    ...
    private Shader createGradientShader() {
        Rect rect = this.rect;
        copyBounds(rect);
        float height = this.f7299a / rect.height();
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        float min = Math.min(this.shapeAppearanceModel.getTopLeftCornerSize().getCornerSize(a()), this.rectF.width() / 2.0f);
        if (this.shapeAppearanceModel.isRoundRect(a())) {
    ...
    public void getOutline(@NonNull Outline outline) {
        if (this.shapeAppearanceModel.isRoundRect(a())) {
            outline.setRoundRect(getBounds(), this.shapeAppearanceModel.getTopLeftCornerSize().getCornerSize(a()));
            return;
        }
        copyBounds(this.rect);
        this.rectF.set(this.rect);
    ...
    public boolean getPadding(@NonNull Rect rect) {
        if (this.shapeAppearanceModel.isRoundRect(a())) {
            int round = Math.round(this.f7299a);
